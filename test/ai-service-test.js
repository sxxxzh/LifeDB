...
async function testAIService() {
  ...
  for (let i = 0; i < testTexts.length; i++) {
    ...
    try {
      ...
      console.log(`  AI分析: ${result.ai_analyzed ? 'OK' : 'FAIL'}`);
  ...
  console.log('\n使用说明:');
  console.log('1. 设置环境变量 DEEPSEEK_API_KEY');
  console.log('2. 可选设置 DEEPSEEK_BASE_URL (默认: https://api.deepseek.com/v1)');
  console.log('3. 在创建节点时设置 use_ai=true 启用AI分析');
  console.log('4. 用户也可以手动设置 ai_emotion, ai_importance, ai_tags 覆盖AI结果');

  console.log('\n测试 AI 服务连接...');
  const isConnected = await aiService.testConnection();
  console.log(`连接状态: ${isConnected ? 'OK' : 'FAIL'}`);

  if (!isConnected) {
    console.log('\n连接失败，请检查:');
    console.log('- DeepSeek API 密钥是否正确设置');
